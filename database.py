...
def create_acquisitions():
    ...
    print(len(max(banks, key=lambda x: len(x))))
    print(len(banks))
    ...
    create_acquisition = """DROP TABLE IF EXISTS Acquisition;
                            CREATE TABLE Acquisition (
                                ...
                                dealDate DATE,
                                PRIMARY KEY (dealNo),
                                FOREIGN KEY (acquiror) REFERENCES ABank(bankID) ON DELETE CASCADE,
                                FOREIGN KEY (target) REFERENCES ABank(bankID) ON DELETE CASCADE);"""
    ...
    acquisitions_q = ",".join(["(" +
                               ...
                               else "'" + str(datetime(*xlrd.xldate_as_tuple(sheet.cell_value(rowx=i, colx=13), book.datemode)).date()) + "'") +
                               ")" for i in range(1, sheet.nrows)])
    #print(insert_acquisition % acquisitions_q)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(insert_acquisition % acquisitions_q)
            connection.commit()
...
def create_database():
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("DROP DATABASE IF EXISTS Banks")
            cursor.execute("CREATE DATABASE Banks")
            connection.commit()
    finally:
        connection.close()
    print("creating acquisitions")
    create_acquisitions()
